...
class Cocrystal(object):
    ...
    def descriptors(self, includeSandP=True, ccdc=True, charge_model='eem2015bm'):
        desc1 = self.coformer1.descriptors(includeSandP=includeSandP, ccdc=ccdc, charge_model=charge_model)
        desc2 = self.coformer2.descriptors(includeSandP=includeSandP, ccdc=ccdc, charge_model=charge_model)
    ...
    def CCGraphTensor(self, t_type='OnlyCovalentBond', hbond=True, pipi_stack=False, contact=False):
        ...
        t_interaction = self.InteractionTensor(hbond=hbond, pipi_stack=pipi_stack, contact=contact)
        if t_interaction is None:
            return CCG
        else:
            return np.append(CCG, t_interaction, axis=1)
