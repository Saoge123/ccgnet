...
def make_fc_layer(inputs, out_size, with_act_func=True, with_bn=True, is_training=1, num_updates=None, name=None):
    with tf.variable_scope(name, default_name='FullConnect') as scope:
...
def batch_node_number(numbers):
    with tf.variable_scope('BatchNodeNumber') as scope:
...
def batch_node_range(numbers):
    with tf.variable_scope('BatchNodeRange') as scope:
        init_counter = tf.constant(0,dtype=tf.as_dtype(numbers.dtype))
        init_tensor =  tf.zeros(0, dtype=tf.as_dtype(numbers.dtype))
        c = lambda i, m: tf.less(i, tf.size(numbers,out_type=tf.as_dtype(numbers.dtype)))
...
def segment_softmax(batch_node_num, gate, name=None):
    with tf.variable_scope(name, default_name='Segment_Softmax') as scope:
        segment_max = tf.segment_max(gate, batch_node_num, name='segment_max')
        ...
        segment_sum = tf.add(tf.gather(tf.segment_sum(exp, batch_node_num), batch_node_num),1e-16, name='segment_sum')
...
def broadcast_global_state(subgraph_size, global_state, V_shape, name=None):
    with tf.variable_scope(name, default_name='Broadcast_Global_State') as scope:
...
def CCGBlock(V, A, global_state, subgraph_size, no_filters=64, mask=None, num_updates=None, is_training=1, name=None):
    with tf.variable_scope(name, default_name='CCGBlock') as scope:
        ...
        global_state_transfer = tf.nn.relu(global_state_transfer)
...
def multi_head_global_attention(V, graph_size, num_head=5, is_training=1, concat=True, multi_layer=None, name=None):
    with tf.variable_scope(name, default_name='Multi_Head_Global_Attention') as scope:
        ...
        if concat:
            ...
            V = tf.segment_sum(V, node_numbers)
...
def Set2Set(x, graph_size, time_steps=3, name=None):
    with tf.variable_scope(name, default_name='Set2Set') as scope:
        node_numbers = batch_node_number(graph_size)
        node_range = batch_node_range(graph_size)
        node_index = tf.stack([node_numbers, node_range], axis=1)
        ...
        for step in range(time_steps):
            ...
            r = tf.segment_sum(tf.multiply(a,x), node_numbers)
