...
def make_softmax_layer(V, axis=1, name=None):
    with tf.compat.v1.variable_scope(name, default_name='Softmax') as scope:
        ...
        prob = tf.divide(exp, tf.reduce_sum(exp, axis=axis, keepdims=True), name='prob')
        return prob
...
def make_fc_layer(inputs, out_size, act_func=tf.nn.relu, with_bn=True, is_training=1, num_updates=None, name=None):
...
def CCGBlock(V, A, global_state, subgraph_size, no_filters=64, act_func=tf.nn.relu, mask=None, num_updates=None, is_training=1, name=None):
    with tf.compat.v1.variable_scope(name, default_name='CCGBlock') as scope:
        ######## transfer global_state #########
        dim_global_state = global_state.get_shape()[-1].value
        global_state = tf.reshape(global_state, [-1, int(dim_global_state/2)])
        global_state_transfer = make_fc_layer(global_state, no_filters, act_func=act_func, with_bn=False, is_training=is_training)
        ...
        global_state_transfer = act_func(global_state_transfer)
        global_state_transfer = tf.reshape(global_state_transfer, [-1, no_filters*2])
    ...
    with tf.compat.v1.variable_scope(name, default_name='Multi_Head_Global_Attention') as scope:
        ...
        if multi_layer:
            for i in multi_layer:
                gathered_V = make_fc_layer(gathered_V, i, is_training=is_training, with_bn=True)
...
def ReadoutFunction(V, global_state, graph_size, num_head=5, is_training=1, concat=True, multi_layer=None, name=None):
    V = multi_head_global_attention(V, graph_size, num_head=num_head, is_training=is_training, concat=concat, multi_layer=multi_layer, name=name)
    dim_global_state = global_state.get_shape()[-1].value
    global_state = tf.reshape(global_state, [-1, int(dim_global_state)])
