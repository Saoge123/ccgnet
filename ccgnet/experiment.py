...
def create_graph_placeholders(dataset, use_desc=True, with_tags=True, with_attention=True, use_subgraph=False):
    ...
    V = tf.placeholder(tf.as_dtype(dataset[0].dtype), shape=V_shape, name='V_input')
    placeholders.append(V)
    A_shape = [None] + list(dataset[1].shape[1:])
    A = tf.placeholder(tf.as_dtype(dataset[1].dtype), shape=A_shape, name='AdjMat_input')
    placeholders.append(A)
    labels_shape = [None]
    labels = tf.placeholder(tf.as_dtype(dataset[2].dtype), shape=labels_shape, name='labels_input')
    placeholders.append(labels)
    mask_shape = [None] + list(dataset[3].shape[1:])
    masks = tf.placeholder(tf.as_dtype(dataset[3].dtype), shape=mask_shape, name='masks_input')
    placeholders.append(masks)
    if with_attention:
        graph_size_shape = [None]
        graph_size = tf.placeholder(tf.as_dtype(dataset[4].dtype), shape=graph_size_shape, name='graph_size_input')
        placeholders.append(graph_size)
    if with_tags:
        tags_shape = [None]
        tags = tf.placeholder(tf.as_dtype(dataset[5].dtype), shape=tags_shape, name='tags_input')
        placeholders.append(tags)
    if use_desc:
        global_state_shape = [None] + list(dataset[6].shape[1:])
        global_state = tf.placeholder(tf.as_dtype(dataset[6].dtype), shape=global_state_shape, name='global_state_input')
        placeholders.append(global_state)
    if use_subgraph:
        subgraph_size_shape = [None, 2]
        subgraph_size = tf.placeholder(tf.as_dtype(dataset[7].dtype), shape=subgraph_size_shape, name='subgraph_size_input')
        placeholders.append(subgraph_size)
    return placeholders

def create_fc_placeholders(dataset):
    embedding_shape = [None] + list(dataset[0].shape[1:])
    embedding = tf.placeholder(tf.as_dtype(dataset[0].dtype), shape=embedding_shape, name='Mol_Embedding')
    labels_shape = [None]
    labels = tf.placeholder(tf.as_dtype(dataset[1].dtype), shape=labels_shape, name='labels_input')
    tags_shape = [None]
    tags = tf.placeholder(tf.as_dtype(dataset[2].dtype), shape=labels_shape, name='tags_input')
    try:
        desc_shape = [None] + list(dataset[3].shape[1:])
        desc = tf.placeholder(tf.as_dtype(dataset[3].dtype), shape=desc_shape, name='desc_input')
...
def create_input_variable(inputs):
    ...
    for i in range(len(inputs)):
        placeholder = tf.placeholder(tf.as_dtype(inputs[i].dtype), shape=inputs[i].shape)
...
def create_loss_function(V, labels, is_training):
    with tf.variable_scope('loss') as scope:
        ...
        max_acc = tf.cond(is_training, lambda: tf.assign(max_acc_train, tf.maximum(max_acc_train, accuracy)), lambda: tf.assign(max_acc_test, tf.maximum(max_acc_test, accuracy)))
    ...
    return tf.add_n(tf.get_collection('losses')), reports
...
def make_train_step(loss, global_step, optimizer='adam', starter_learning_rate=0.1, learning_rate_step=1000, learning_rate_exp=0.1, reports=None):
    ...
    with tf.control_dependencies(update_ops):
        if optimizer == 'adam':
            train_step = tf.train.AdamOptimizer().minimize(loss, global_step=global_step, name='train_step')
        else:
            learning_rate = tf.train.exponential_decay(starter_learning_rate, global_step, learning_rate_step, learning_rate_exp, staircase=True)
            train_step = tf.train.MomentumOptimizer(learning_rate, 0.9).minimize(loss, global_step=global_step, name='train_step')
...
def make_batch(data, epoch, batch_size, with_shuffle=True, name=None):
    with tf.variable_scope(name, default_name='input_slice') as scope:
        inputs = []
        for i in data:
            ph = tf.placeholder(tf.as_dtype(i.dtype), shape=i.shape)
            inputs.append(ph)
...
class Model(object):
    def __init__(self, model, train_data, test_data,val=False, val_data=None,  build_fc=False, 
    ...
    def fit(self, num_epoch=100, 
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            ###################### training record #########################
            self.test_max_acc = {}
            self.test_max_acc['test_acc'] = []
            self.test_max_acc['test_cross_entropy'] = []
            self.test_max_acc['train_acc'] = []
            self.test_max_acc['train_cross_entropy'] = []
            if bacc_metric:
                self.test_max_acc['test_bacc'] = []
            ###################### configure model saver #######################
            var_list = [var for var in tf.global_variables() if "moving" in var.name]
            var_list += [var for var in tf.global_variables() if "Moving" in var.name]
            var_list += tf.trainable_variables()
            saver = tf.train.Saver(var_list=var_list, max_to_keep=max_to_keep)
            ####################################################################
            if save_att:
            ...
            try:
                for epo in range(num_epoch):
                    ...
                    for b in range(self.test_batch_num_per_epoch):
                        ...
                        test_tags.append(test_batch[5])
                        test_labels.append(test_batch[2])
                        if save_att:
                    ...
                    self.test_max_acc['test_acc'].append(test_acc/self.test_batch_num_per_epoch)
                    self.test_max_acc['test_cross_entropy'].append(test_loss/self.test_batch_num_per_epoch)
                    ...
                    if bacc_metric:
                        ...
                        self.test_max_acc['test_bacc'].append(test_bacc)
                        save_metric = test_bacc
                    else:
                        save_metric = self.test_max_acc['test_acc'][-1]
                    ###########################  save model ############################
                    if save_metric > test_acc_cutoff:
                        early_stop_counter = 0
                        test_acc_cutoff = save_metric
                        ...
                        if self.val:
                            ...
                            self.test_max_acc['val_acc'] = val_reports['accuracy']
                        ######################## save model information ########################
                        if save_info:
                            if save_att:
                                att = str(att.tolist())
                                model_info = ['step:{}'.format(epo),
                                              'test_acc:{}'.format(test_reports['accuracy']),
                                              'test_cross_entropy:{}'.format(test_reports['cross_entropy']),
                            ...
                            if bacc_metric:
                                model_info.insert(2,'test_bacc:{}'.format(save_metric))
                    ...
                    if silence == False:
                        ...
                        print_content = '## Epoch {} ==> Train Loss:{:.5f}, Train Acc:{:.2f}, Test Loss:{:.5f}, Test Acc:{:.2f}, Elapsed Time:{:.2f} s'
                        print_content = print_content.format(epo,
                                                             self.test_max_acc['train_cross_entropy'][epo],
                                                             self.test_max_acc['train_acc'][epo]*100,
                                                             self.test_max_acc['test_cross_entropy'][epo],
                                                             self.test_max_acc['test_acc'][epo]*100,
                                                             elapsed_time)
                        print(print_content)
                    if early_stop_counter == early_stop_cutoff and early_stop != False:
                        print('trigger early stopping, model training will be ended ...')
                        break
