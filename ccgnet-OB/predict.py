...
class Inference(object):
    def __init__(self, table_path, mol_dir='./', mol_file_type='sdf'):
        meta_file ='./inference/CC_Model/Model_0/TestAcc-99.22-29.meta'
...
def GetCoformerSmiles(cc_table, mol_dir='./coformers', fmt=None):
...
def main(table, mol_dir, fmt='sdf', model_type='cc', xlsx_name='Result.xlsx'):
    ...
    if model_type == 'cc':
        paths = glob.glob('./inference/CC_Model/*/')
    elif model_type == 'ecc':
        paths = glob.glob('./inference/ECC_Model/*/')
    score_pool = []
    for p in paths:
