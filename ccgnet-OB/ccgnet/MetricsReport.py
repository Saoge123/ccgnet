...
def model_metrics_report(path, is_print=False, tofixed=4, DNN_info=False):
    ...
    for model in models:
        ...
        for fold in folds:
            ...
            #fr = open(d[sorted(d.keys())[-2]]).readlines()
            if 'bacc' in fr[2]:
                labels = eval(fr[6])
            else:
                labels = eval(fr[5])
...
def ValidReport(path, each_fold=0):
    path = path+'/*/*val*'
    val_list = glob.glob(path)
    BACC = []
    TP = []
    Nacc = []
    Pacc = []
    ...
    F1 = []
    for i in val_list:
        fr = open(i).read().split('\n')
        labels = eval(fr[1])
        pred = [func(j) for j in eval(fr[2])]
        bacc = balanced_accuracy_score(labels, pred)
        tp = precision_score(labels, pred)
        ...
        F1.append(f1)
        if each_fold:
            print(i, nacc, pacc, bacc)
    print('BACC: {:.2f}(±{:.2f})'.format(np.array(BACC).mean()*100, np.array(BACC).std()*100))
    print('TP: {:.2f}(±{:.2f})'.format(np.array(TP).mean()*100, np.array(TP).std()*100))
    print('Nacc: {:.2f}(±{:.2f})'.format(np.array(Nacc).mean()*100, np.array(Nacc).std()*100))
    print('Pacc: {:.2f}(±{:.2f})'.format(np.array(Pacc).mean()*100, np.array(Pacc).std()*100))
    print('Recall: {:.2f}(±{:.2f})'.format(np.array(Recall).mean()*100, np.array(Recall).std()*100))
    print('F1: {:.2f}(±{:.2f})'.format(np.array(F1).mean()*100, np.array(F1).std()*100))